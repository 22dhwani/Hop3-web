...
export default function Dashboard() {
  ...
  const { data:userPostData, isLoading:isPostLoading, error:getUserPostError,refetch:getUserPost } = useQuery(['getUserPost',pagination.limit,pagination.page_number], getPostForUser,{enabled:false});
  const { data:adminPostData,refetch:getAdminPost } = useQuery(['getAdminPost',pagination.limit,pagination.page_number,postFilter], getPostForAdmin,{enabled:false});
    ...
    <div className={styles.container}>
      <header className={styles.header}>
        <div className={styles.right}>
          <div className={styles.profile}>
            ...
            <button className={styles.sharebutton} onClick={onClickShareExperience}>Share Experience</button>
      ...
      <div>
        ...
        {allPost.map((data: IPostDataItem) => (
          <PostItem key={data.id} data={data} isAdmin={isAdmin} onRefresh={onRefresh}/>
        ))}
      </div>
    </div>
  );
...
const PostItem = (props: { data: IPostDataItem ,isAdmin:boolean, onRefresh?: () => void }) => {
        ...
        <div className={styles.description}>
          ...
          <div className={styles.selectwrapper}>
            ...
            {isAdmin &&
                ...
                >
                  {status.map((name) => (
                      <MenuItem key={name} value={name} style={{display: name === 'Pending' ? 'none' : 'flex'}}>
                        {name}
                      </MenuItem>
                  ))}
                </Select>
